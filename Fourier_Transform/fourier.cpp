...
int main(){
    Mat inputImg = imread("mini.jpg", CV_LOAD_IMAGE_GRAYSCALE);
    Mat padded;                         
    ...
    imshow("輸入圖", inputImg);    
    imshow("頻譜", magI);
    // write to output1, but nothing can find
    imwrite("output1.jpg", magI);
    ...
    imshow("逆向求輸入圖",ifft);  
    // write to output2, but nothing can find
    imwrite("output2.jpg",ifft);  
    waitKey();
