...
int main(){
    Mat img = imread("line.jpg",CV_LOAD_IMAGE_GRAYSCALE);
    Mat result1 = imread("line.jpg",CV_LOAD_IMAGE_COLOR);
    Mat result2 = imread("line.jpg",CV_LOAD_IMAGE_COLOR);
    ...
    imwrite("output1.jpg", result1);
    imwrite("output2.jpg", result2);
    waitKey(0);  
...
void calcLinesP(const Mat &input, std::vector<Vec4i> &lines){ 
    ...
    imwrite("test.jpg", contours);
    lines.clear(); 
    HoughLinesP(contours, lines, 1, CV_PI/180, 50); 
    imwrite("test2.jpg", contours);
}
